    ...
    for ext in exts:
        ...
        for file in files:
            if file[0].split('.')[1] == ext:
                ...
                if file[2] == 'B': temp_list_B.append(int(file[1]))
                if file[2] == 'KB': temp_list_KB.append(int(file[1]))
                if file[2] == 'MB': temp_list_MB.append(int(file[1]))
                if file[2] == 'GB': temp_list_GB.append(int(file[1]))
...
def result_info(dict):
    ...
    unit = 'B'
    if summary/1024 >= 1: summary, unit = round(summary/1024), 'KB'
    if summary/1024 >= 1: summary, unit = round(summary/1024), 'MB'
    if summary/1024 >= 1: summary, unit = round(summary/1024), 'GB'
